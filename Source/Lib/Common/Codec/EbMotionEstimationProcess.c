                ...
                for (y_lcu_index = yLcuStartIndex; y_lcu_index < yLcuEndIndex; ++y_lcu_index) {
                    for (x_lcu_index = xLcuStartIndex; x_lcu_index < xLcuEndIndex; ++x_lcu_index) {
                        ...
                        if (picture_control_set_ptr->enable_hme_level0_flag) {
                            ...
                            uint32_t bufferChromaIndex = ((sixteenth_decimated_picture_ptr->origin_y >> 1)+ (sb_origin_y >> 3)) * (sixteenth_decimated_picture_ptr->stride_y >> 1) + (sixteenth_decimated_picture_ptr->origin_x >> 1) + (sb_origin_x >> 3);
#endif
                            {
...
#if HME_LEVEL_O_CHROMA
                                uint8_t  *frameCbPtr = &sixteenth_decimated_picture_ptr->buffer_cb[bufferChromaIndex];
                                uint8_t  *localCbPtr = context_ptr->me_context_ptr->sixteenth_cb_sb_buffer;

                                uint8_t  *frameCrPtr = &sixteenth_decimated_picture_ptr->buffer_cr[bufferChromaIndex];
                                ...
                                if (context_ptr->me_context_ptr->hme_search_method == FULL_SAD_SEARCH) {
                                    ...
                                    for (lcuRow = 0; lcuRow < (sb_height >> 3); lcuRow += 1) {
                                        ...
                                        localCbPtr += 8;
                                        frameCbPtr += sixteenth_decimated_picture_ptr->stride_cb;

                                        EB_MEMCPY(localCrPtr, frameCrPtr, (sb_width >> 3) * sizeof(uint8_t));
                                        localCrPtr += 8;
                                        frameCrPtr += sixteenth_decimated_picture_ptr->stride_cr;
                                    }
#endif
                                }
                                else {
